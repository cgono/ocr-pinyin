...
- [x] Update `backend/.env.example` (AC: 1)
  ...
  - [x] Replace AWS vars with `OCR_PROVIDER=google_vision` and `GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account-key.json`
- [x] Update `docker-compose.yml` (AC: 1)
  - [x] Add credentials file volume mount to the backend service per §4.4
...
- Client: `vision.ImageAnnotatorClient()` — reads credentials automatically from `GOOGLE_APPLICATION_CREDENTIALS` env var (service account JSON file path)
...
2. Set `GOOGLE_APPLICATION_CREDENTIALS=/absolute/path/to/key.json` in `backend/.env`
3. The `google-cloud-vision` library reads this env var automatically via `google-auth`

### docker-compose.yml Update

Add credentials volume mount under the `backend` service (after the existing `./backend:/app` volume):
```yaml
volumes:
  - ./backend:/app
  - ${GOOGLE_APPLICATION_CREDENTIALS}:${GOOGLE_APPLICATION_CREDENTIALS}:ro
```
This mounts the host credentials file into the container at the same path, so `GOOGLE_APPLICATION_CREDENTIALS` works identically inside and outside Docker.
...
3. Start backend with `OCR_PROVIDER=google_vision GOOGLE_APPLICATION_CREDENTIALS=...`
